drop table if exists STG_DSD_CANADA.DEMANTRA;

CREATE TABLE if not exists  STG_DSD_CANADA.DEMANTRA
AS
SELECT
	...
	FROM
	     (
			...
			FROM
			     ...
			     LEFT JOIN (select *, regexp_replace(agency,'[^[:digit:]]','') as codigo_agencia from STG_DSD_CANADA.AGM001_HIST) AGM001
			          ON AGY0043.CODIGO_AGENCIA = CAST(AGM001.CODIGO_AGENCIA AS INTEGER) AND AGY0043.CLICOD = AGM001.CLICOD
