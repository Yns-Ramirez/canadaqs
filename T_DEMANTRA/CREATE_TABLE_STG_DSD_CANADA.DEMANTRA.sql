...
FROM
	...
	FROM
	     (
			...
			FROM
			     ...
			     LEFT JOIN
			               (
			                    ...
			                    INNER JOIN
			                              (
			                                   SELECT YEAR_OF_CALENDAR, WEEK_OF_YEAR, MIN(CALENDAR_DATE) AS FIRST_CALENDAR_DATE FROM CP_SYS_CALENDAR.CALENDAR
			                                   GROUP BY 1,2
			...
			WHERE trim(SUBSTR(DC.VALOR,1,7)) IS NOT NULL
			AND AGY0043.AG03FC BETWEEN TO_DATE(DATE_SUB(FROM_UNIXTIME(UNIX_TIMESTAMP()),7)) AND TO_DATE(DATE_SUB(FROM_UNIXTIME(UNIX_TIMESTAMP()),1))
